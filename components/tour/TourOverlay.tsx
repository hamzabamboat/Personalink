...
export function TourOverlay() {
  ...
  const view = tour.view
  const open = tour.isActive && !!view

  const isMobile = viewport.width < 640
  const spotlightRect = view?.mode === 'spotlight' ? tour.targetRect : null
  const hasSpotlight = !!spotlightRect

  // Card position (none of these branches dereference `view`, so they're safe
  // even when the tour is closed — the card simply isn't rendered then).
