...
export async function POST(request: NextRequest) {
  ...
  if (grantType === 'authorization_code') {
    ...
    // single-use
    await supabaseAdmin.from('oauth_auth_codes').update({ consumed_at: new Date().toISOString() }).eq('code', sha256(code))
  ...
  if (grantType === 'refresh_token') {
    ...
    await supabaseAdmin.from('oauth_tokens').update({ revoked_at: new Date().toISOString() }).eq('id', row.id)
    return issueTokens(row.client_id as string, row.user_id as string, row.scope as string)
  }
