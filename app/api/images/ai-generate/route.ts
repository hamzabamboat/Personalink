  ...
  try {
    ...
    for (let i = 0; i < wantVariations; i++) {
      ...
      try {
        ...
        const buffer = await sharp(Buffer.from(b64, 'base64')).resize(w, h, { fit: 'cover' }).flatten({ background: '#ffffff' }).jpeg({ quality: 85 }).toBuffer()

        const storagePath = `${user.id}/ai-${Date.now()}-${i}-${crypto.randomUUID()}.jpg`
        ...
        const { data: imageRow } = await supabaseAdmin
          ...
          .insert({
            ...
            file_name: `ai-generated-${Date.now()}.jpg`,
