...
import { getAllCompetitors } from '@/lib/competitor-data'
...
export default async function VsIndexPage() {
  const competitors = getAllCompetitors(await getUsdInrRate())
    ...
    <LandingShell>
      ...
      <section style={{ ...wrap, paddingBottom: 'clamp(32px,6vw,56px)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 16 }}>
          {competitors.map((c) => (
            <Link key={c.slug} href={`/vs/${c.slug}`} style={{ display: 'block', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 'var(--r-lg)', padding: 22, textDecoration: 'none', boxShadow: 'var(--sh-1)' }}>
